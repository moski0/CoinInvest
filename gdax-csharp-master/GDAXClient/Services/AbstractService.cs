...
namespace GDAXClient.Services
{
    public abstract class AbstractService
    {
            ...
            string content = null)
        {
            ...
            IEnumerable<string> firstPageAfterCursorId;
            if (!httpResponseMessage.Headers.TryGetValues("cb-after", out firstPageAfterCursorId))
            {
                return pagedList;
            }
        ...
        private async Task<IList<IList<T>>> GetAllSubsequentPages<T>(string uri, string firstPageAfterCursorId)
        {
            ...
            while (true)
            {
                ...
                IEnumerable<string> cursorHeaders;
                if (!subsequentHttpResponseMessage.Headers.TryGetValues("cb-after", out cursorHeaders))
                {
                    break;
                }
